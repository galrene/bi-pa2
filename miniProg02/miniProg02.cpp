...
class CTimeStamp
{
  ...
  size_t m_Sec;
public:
  CTimeStamp ( size_t yr, size_t month, size_t day, size_t hr, size_t min, size_t sec ) : m_Year(yr), m_Month(month), m_Day(day), m_Hour(hr), m_Min(min), m_Sec(sec) {}
};
class CContact
{
  ...
  int m_User2;
public:
  CContact ( CTimeStamp stamp, int user1, int user2 ) : m_TimeStamp(stamp), m_User1(user1), m_User2(user2) {}
};
class CEFaceMask
{
  // TODO
